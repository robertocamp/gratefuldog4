...
// eks managed nodes will peform self-managed upgrades
...
module "irsa_role" {
  ...
  role_name = "adot-role"
  role_policy_arns = {
    AmazonPrometheus = "arn:aws:iam::aws:policy/AmazonPrometheusRemoteWriteAccess"
  ...
  oidc_providers = {
    ex = {
      provider_arn               = module.eks.oidc_provider_arn
      namespace_service_accounts = ["opentelemetry-operator-syste:adot-collector"]
    }
  }
}
